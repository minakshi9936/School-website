      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
          {cards.map((card, index) => {
            ...
            return (
              <AnimatedCard key={index} delay={index * 0.1} className="p-6 md:p-8">
                <div
                  className={`w-12 h-12 md:w-16 md:h-16 rounded-lg bg-gradient-to-br ${card.color} flex items-center justify-center mb-4 md:mb-6`}
                >
                  <Icon className="w-6 h-6 md:w-8 md:h-8 text-white" />
                </div>
                <h3 className="text-xl md:text-2xl font-bold mb-4 text-gray-900">{card.title}</h3>
                <p className="text-gray-600 leading-relaxed text-sm md:text-base">{card.description}</p>
              </AnimatedCard>
            );
          })}
        </div>

        <div className="mt-16 bg-gradient-to-r from-blue-600 to-blue-700 rounded-2xl p-6 sm:p-8 md:p-12 text-white">
          <div className="max-w-3xl mx-auto text-center">
            <h3 className="text-2xl sm:text-3xl font-bold mb-4">Why Choose BrightFuture Academy?</h3>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {['Smart Classrooms', 'Sports Facilities', 'Science Labs', 'Arts & Culture'].map(
                (facility, index) => (
                  <div
                    key={index}
                    className="bg-white/10 backdrop-blur-sm rounded-lg p-3 md:p-4 hover:bg-white/20 transition-colors"
                  >
                    <p className="font-semibold text-sm md:text-base">{facility}</p>
                  </div>
                )
              )}
            </div>
